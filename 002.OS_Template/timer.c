...
void Timer0_Delay(int mtime) //delay 주는 함수
{
...
void Timer0_Int_Delay(int en, int mtime) //delay 주는 함수, mtime이 주기적으로 주는 것, 주기적으로 IRQ expection으로 HandlerIRQ로 감 crtt0.s
{
	if(en)
	{
		Macro_Write_Block(rTCFG0,0xff,(int)(TIMER_BASECLK/16./50000+0.5)-1,0);
